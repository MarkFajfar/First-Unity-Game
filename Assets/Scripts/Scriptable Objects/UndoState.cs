...
namespace NavajoWars
{
    ...
    gs.AP = 10;
    ui.displayText("gs.AP before = " + gs.AP);
    //THIS WORKS, UndoState SO must have all the same fields as GameState
    UndoState undoState = ScriptableObject.CreateInstance<UndoState>();
    JsonUtility.FromJsonOverwrite(sd, undoState);
    string undoString = JsonUtility.ToJson(undoState);
    JsonUtility.FromJsonOverwrite(undoString, gs);
    ui.addText("gs.AP after = " + gs.AP);*/
...
public class UndoState : ScriptableObject
    {
        ...
        public GameStep currentOpsStep;
        // does not need to be a property, but if it is cannot use simple {get; set;}
